...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(100.0, 30.0),  # All numbers are approximate.
                rg.Point(90.0, 47.3),
                rg.Point(70.0, 47.3),
                rg.Point(60.0, 30.0),
                rg.Point(70.0, 12.7),
                rg.Point(90.0, 12.7)]
    circle = rg.Circle(rg.Point(80, 30), 20)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 125)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    new_point = generate_points_on_circle(circle, number_of_points)
    for k in range(len(new_point)):
        new_point[k].attach_to(window)
        new_circle = rg.Circle(new_point[k], 10)
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'PIZZA test 4:  6 slices, thin (thickness=4) yellow lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 6, 'yellow', 4)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(new_points)):
        ...
        new_line = rg.Line(circle.center, new_points[k])
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = 'POLYGON test 3 3 segments with thick green lines'
    window = rg.RoseWindow(550, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 3, 'green', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(new_points)):
        new_points[k].attach_to(window)
        new_line = rg.Line(new_points[k-1], new_points[k])
        new_line.attach_to(window)
        new_line.color = color
        new_line.thickness = thickness
    window.render()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('FANCY POLYGON test 4:  80 cyan lines on black circle,'
             + ' hops = 21.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(280, 165), 150)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 80, 21, 'cyan', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    """
    circle.attach_to(window)
    new_point = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(new_point)):
        if k + hops_to_next_point % len(new_point) != 0:
            point = 0 + ((k + hops_to_next_point) % len(new_point))

        else:
            point = k + hops_to_next_point

        new_point[k].attach_to(window)
        new_line = rg.Line(new_point[k], new_point[point])
        new_line.attach_to(window)
        new_line.thickness = thickness
        new_line.color = color
        new_line.arrow = 'last'
